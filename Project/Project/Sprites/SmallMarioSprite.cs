...
namespace Project.Sprites
{
     ...
     */
    public class SmallMarioSprite : IMarioSprite
    {
        ...
        {
            Texture = texture;
            standingRightMap = Config.GetStandingRightMapForSmallMarioSprite();
            runningRightMap = Config.GetRunningRightMapForSmallMarioSprite();
            jumpingRightMap = Config.GetJumpingRightMapForSmallMarioSprite();
            standingLeftMap = Config.GetStandingLeftMapForSmallMarioSprite();
            runningLeftMap = Config.GetRunningLeftMapForSmallMarioSprite();
            jumpingLeftMap = Config.GetJumpingLeftMapForSmallMarioSprite();
            crouchingLeftMap = Config.GetCrouchingLeftMapForSmallMarioSprite();
            crouchingRightMap = Config.GetCrouchingRightMapForSmallMarioSprite();
        ...
        public void Update()
        {
            ...
            if (updateCounter >= CALLS_PER_UPDATE)
            {
                updateCounter = 0;
                animationPosition++;
                if (animationPosition >= currentMap.Length / 4)//divide by 4 because each frame has 4 pieces of data
                {
                    animationPosition = 0;
                }
            }
